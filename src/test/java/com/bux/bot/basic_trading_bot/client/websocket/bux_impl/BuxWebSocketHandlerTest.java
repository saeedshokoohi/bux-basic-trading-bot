...
class BuxWebSocketHandlerTest {
    ...
    void testHandleOutputMessage() {
        ...
        outputMessagesList.add(new WebSocketEvent(WebSocketStatusEventType.OUT_MESSAGE,new WebSocketEventMessage(MockData.SUBSCRIBE_TO_MESSAGE_1)));
        outputMessagesList.add(new WebSocketEvent(WebSocketStatusEventType.OUT_MESSAGE,new WebSocketEventMessage(MockData.SUBSCRIBE_TO_MESSAGE_2)));
        ...
        when(session.send(any())).thenReturn(Mono.empty());
          //   when(webSocketEventBus.subscribeOnOutput(any())).thenReturn(outputMessages);
        //when
         outputMessages.subscribe(e->{webSocketEventBus.emitToOutput(e);}) ;
         buxWebSocketHandler.handle(session);
        //then
        ArgumentCaptor<String> captor = ArgumentCaptor.forClass(String.class);

        verify(session.textMessage(any()),times(2));
//        verify(session).textMessage(captor.capture());
//        List<String> sendMessages = captor.getAllValues();
//        assertThat(sendMessages).matches(m->
//            outputMessagesList.contains(m)
//        );
