...
public class EventAggregatorService {
  ...
  public EventAggregatorService(
    ...
    private void passEventsFromWebSocketEventBusToGlobalEventBus() {
        this.webSocketEventBus.subscribeOnConnection(e->{
            globalEventBus.emit(new GlobalEvent<WebSocketEvent>(GlobalEventType.WEBSOCKET_CONNECTION,this,e));
        });
        this.webSocketEventBus.subscribeOnInput(e->{
            globalEventBus.emit(new GlobalEvent<WebSocketEvent>(GlobalEventType.WEBSOCKET_INPUT,this,e));
        });
        this.webSocketEventBus.subscribeOnOutput(e->{
            globalEventBus.emit(new GlobalEvent<WebSocketEvent>(GlobalEventType.WEBSOCKET_OUTPUT,this,e));
        });
    }
}
